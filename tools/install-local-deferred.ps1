...
$heldLocks = @()
$modId = 'tropimon_events'
$filePrefix = 'TropimonEvents'
...
try {
    ...
    # Snapshot is taken when the update is armed, not after Minecraft exits.
    $prepared=@{}
    foreach($candidate in (Get-ChildItem -LiteralPath $mods -Filter '*.jar' -File)){
        $manifest=Read-Manifest $candidate.FullName
        if($manifest.id -ceq $modId){$prepared[$candidate.FullName]=Get-Hash $candidate.FullName}
    }
    ...
    if ($installed.Count -gt 1) { throw 'Multiple installed copies are ambiguous.' }
    if ($installed.Count -ne $prepared.Count) { throw 'Installed mod set changed after preparation.' }
    foreach ($old in $installed) {
        if (!$prepared.ContainsKey($old.FullName) -or (Get-Hash $old.FullName) -ine $prepared[$old.FullName]) {
            throw 'Installed mod changed after preparation.'
        }
    ...
    $backupDir = Join-Path $root ('mod-archive/' + $modId + '-' + [guid]::NewGuid().ToString('N'))
    ...
    try {
        foreach ($old in $installed) {
            $lock = [IO.File]::Open($old.FullName, [IO.FileMode]::Open, [IO.FileAccess]::Read, [IO.FileShare]::Delete)
            $heldLocks += $lock
            if ((Get-FileHash -InputStream $lock -Algorithm SHA256).Hash -ine $prepared[$old.FullName]) { throw 'Target changed immediately before replacement.' }
        ...
        foreach ($lock in $heldLocks) { $lock.Dispose() }
        $heldLocks = @()
        foreach ($entry in $moved) {
            if ((Get-Hash $entry.Backup) -ine $prepared[$entry.Original]) { throw 'Backup integrity mismatch.' }
        }
    } catch {
