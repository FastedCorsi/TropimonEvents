...
$source=Join-Path $delivery 'TropimonEvents-0.1.0+1.21.1-LOCAL.jar'
...
$managedScript=Join-Path $PSScriptRoot 'InstallManagedLocalMod.ps1'
$result=& powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File $managedScript -SourceJar $source -ExpectedModId 'tropimon_events' -LauncherRoot $managedInstance
if($LASTEXITCODE -ne 0){throw 'Managed fixture installation failed'}
$state=($result|Select-Object -Last 1)|ConvertFrom-Json
if($state.state -ne 'installed'){throw 'Managed fixture not installed'}
