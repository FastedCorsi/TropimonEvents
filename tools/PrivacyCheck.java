...
class PrivacyCheck {
    ...
    private void inspect(String location, byte[] bytes, int depth) throws IOException {
        ...
        // Exact upstream Maven metadata reviewed against Maven Central. Preserve public third-party credits.
        if (location.endsWith("!/META-INF/jars/aircompressor-0.27.jar!/META-INF/maven/io.airlift/aircompressor/pom.xml")) {
            try {
                String hash = HexFormat.of().formatHex(java.security.MessageDigest.getInstance("SHA-256").digest(bytes));
                if (hash.equals("5aacda89be38563bdf3bfac79333616c0802ca617070dea66ba649507c994441"))
                    categories.remove("email-review-required");
        ...
        for (int index = 1; index < args.length; index++) {
            ...
            try (ZipFile zip = new ZipFile(artifact.toFile())) {
                ...
                if (metadata == null) check.issues.add(artifact.getFileName() + ": missing-metadata");
                    ...
                    if (!text.contains("\"id\": \"tropimon_events\"") || !AUTHOR.matcher(text).find())
